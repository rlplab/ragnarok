...
from pddl.pddl_types import TypedObject, Type
...
def binarize_task(task: Task) -> Task:
    ...
    binarized_objects.extend(task.objects)

    for atom in task.init:
        if len(atom.args) > 2:
            glue_arg = get_glue_name(atom)
            binarized_objects.append(TypedObject(glue_arg, atom.predicate))
            for i in range(len(atom.args)):
                binarized_predicate_name = binarize_name(atom.predicate, i)
                arg = atom.args[i]
                binarized_init.append(Atom(binarized_predicate_name, (arg, glue_arg)))
        else:
            binarized_init.append(atom)

    for action in task.actions:
        assert len(action.parameters) == action.num_external_parameters
        binarized_parameters = action.parameters
        binarized_precondition = []
        ...
        for condition in action.effects:
            if len(condition.literal.args) > 2:
                assert False
            else:
    ...
    assert len(task.functions) == 0

    for condition in task.goal.parts:
        assert not condition.negated
        if len(condition.args) > 2:
            glue_arg = get_glue_name(condition)
            for i in range(len(condition.args)):
                binarized_predicate_name = binarize_name(condition.predicate, i)
                arg = condition.args[i]
                binarized_goal.append(Atom(binarized_predicate_name, (arg, glue_arg)))
        else:
